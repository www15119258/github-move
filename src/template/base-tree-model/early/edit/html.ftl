<el-dialog @open="beforeOpen" :close-on-click-modal="false" :title="'新增角色'" :visible.sync="show" :before-close="beforeClose" @close="initForm" append-to-body>
    <el-form v-if="form" :model="form" ref="form" :rules="rules">
        ...
        <el-form-item label="{{property.name}}" labelWidth="100px" prop="{{property.id}}">
            <base-ui-early-tree-select v-model="form.{{property.id}}" :props="props" :pk="pk" :service="service" :queryForm="queryForm" :validatorFun="validatorFun" :validatorFunArgs="validatorFunArgs"></base-ui-early-tree-select>
        </el-form-item>
                {{/equals}}
            {{else}}
