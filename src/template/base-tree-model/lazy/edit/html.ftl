<el-dialog @open="beforeOpen" :close-on-click-modal="false" :title="'新增目录'" :visible.sync="show" :before-close="beforeClose" @close="initForm" append-to-body>
    <el-form v-if="form" :model="form" ref="form" :rules="rules">
        {{#if metadata}}
        {{#each metadata.property as |property|}}
            {{#if property.inherit}}
                {{#equals property.id 'parent'}}
        <el-form-item label="{{property.name}}" labelWidth="100px" prop="{{property.id}}">
            <base-ui-lazy-tree-select v-model="form.{{property.id}}" :props="props" :pk="pk" :service="service" :buildQueryForm="buildQueryForm" :validatorFun="validatorFun" :validatorFunArgs="validatorFunArgs"></base-ui-lazy-tree-select>
        </el-form-item>
                {{/equals}}
            {{else}}
